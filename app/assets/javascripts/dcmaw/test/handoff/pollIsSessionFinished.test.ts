...
describe("Download app", () => {
  ...
  afterEach(() => {
    sinon.restore();
    delete require.cache[require.resolve("../../handoff/pollIsSessionFinished")];
  });
  ...
  describe("Polling", () => {
    describe("When I do not post anything to the BE within the polling duration", () => {
      it("I am redirected to the timeoutPath", async () => {
        ...
        require("../../handoff/pollIsSessionFinished")
        global.window.GOVUKFrontend.polling('apiUrl', 'redirectPath', 'timeoutPath', `${pollingDuration}`, `${interval}`, 'sessionId')
